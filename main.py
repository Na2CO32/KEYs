...
CONFIG = {
    ...
    "SESSIONS": [
        ...
        {"name": "第五節", "time": "12:10 - 13:00"},
        {"name": "第六節", "time": "13:10 - 14:00"},
        {"name": "第七節", "time": "14:10 - 15:00"},
        {"name": "第八節", "time": "15:10 - 16:00"},
        {"name": "第九節", "time": "16:10 - 17:00"},
    ]
}
...
async def handle_return(
    ...
    # 尋找匹配的紀錄
    initial_count = len(records[key_id])
    records[key_id] = [
        lease for lease in records[key_id]
        if not (lease.get("phone") == phone and lease.get("date") == target_date_info)
    ]

    if len(records[key_id]) == initial_count:
        return JSONResponse(status_code=400, content={"message": f"❌ 找不到您在 {target_date_info} 的預約紀錄。"})
