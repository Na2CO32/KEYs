...
async def get_current_user(request: Request):
    ...
    if not user:
        # 這裡丟出 401，會被下面的 custom_http_exception_handler 捕捉並跳轉
        raise HTTPException(status_code=401, detail="請先登入")
    return user

# 未登入者自動轉到登入頁面 (攔截 401 錯誤)
...
async def handle_login(password: str = Form(...)):
    # 使用原本 CONFIG 裡的 ALLOWED_PASSWORDS 當作登入密碼
    if password in CONFIG["ALLOWED_PASSWORDS"]:
        response = JSONResponse(content={"status": "success"})
        # 設定 Cookie，有效期 1 天 (86400秒)
        response.set_cookie(key="user_session", value="authenticated", max_age=86400)
        return response
    return JSONResponse(status_code=401, content={"message": "❌ 密碼錯誤，請重新輸入。"})

@app.get("/logout")
async def logout():
    response = HTMLResponse(content="<script>location.href='/login';</script>")
