...
async def handle_return(
    ...
    for lease in records[key_id]:
        if (lease.get("phone") == phone and 
            ...
            target_record = lease
            found = True
            break # 找到最近的一筆「已借出」紀錄並處理

    if not found:
        return JSONResponse(status_code=400, content={"message": "❌ 找不到您名下「已借出」的紀錄。"})

    save_records(records)
    return {"status": "success", "message": f"✅ 已為您提交歸還申請！\n日期: {target_record['date']}"}
